...
if($data=="inviteSetting" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        ...
        [
            ...
            ['text'=>"مقدار پورسانت",'callback_data'=>"saosaoch"]
            ],
        [
...
🌐 subscription : <code>$subLink</code>
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"بنازم خرید جدید ❤️",'callback_data'=>"saosaoch"]
        ],
        ]]);
...
$keys = json_encode(['inline_keyboard'=>[
    [
        ['text'=>"به به تمدید 😍",'callback_data'=>"saosaoch"]
        ],
    ]]);
...
if($response->success){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"اخیش یکی زمان زد 😁",'callback_data'=>"saosaoch"]
            ],
        ]]);
sendMessage("
...
if($response->success){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"اخیش یکی حجم زد 😁",'callback_data'=>"saosaoch"]
            ],
        ]]);
sendMessage("
...
elseif($payType == "RENEW_SCONFIG"){
    ...
    editKeys(json_encode(['inline_keyboard'=>[
		    [['text'=>"پرداخت انجام شد",'callback_data'=>"saosaoch"]]
		    ]]));
}else{
...
\n🌐 subscription : <code>$subLink</code>";
    ...
    unset($markup[count($markup)-1]);
    $markup[] = [['text'=>"✅",'callback_data'=>"saosaoch"]];
    $keys = json_encode(['inline_keyboard'=>array_values($markup)],488);
    ...
    if($admin != $from_id){ 
        $keys = json_encode(['inline_keyboard'=>[
            [
                ['text'=>"به به 🛍",'callback_data'=>"saosaoch"]
            ],
            ]]);
...
if($data=="availableServers"){
    ...
    $keys = array();
    $keys[] = [
        ['text'=>"تعداد باقیمانده",'callback_data'=>"saosaoch"],
        ['text'=>"پلن",'callback_data'=>"saosaoch"],
        ['text'=>'سرور','callback_data'=>"saosaoch"]
        ];
    while($file_detail = $serversList->fetch_assoc()){
        ...
        if($name->num_rows>0){
            ...
            $keys[] = [
                ['text'=>$acount . " اکانت",'callback_data'=>"saosaoch"],
...
if($data=="availableServers2"){
    ...
    while($file_detail2 = $serversList->fetch_assoc()){
        ...
        if($name->num_rows>0){
            ...
            $keys[] = [
                ['text'=>$acount2 . " اکانت",'callback_data'=>"saosaoch"],
                ['text'=>$title2??" ",'callback_data'=>"saosaoch"],
                ];
        }
    }
    $keys[] = [['text'=>$buttonValues['back_button'],'callback_data'=>"mainMenu"]];
...
if(preg_match('/^agencyDecline(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    editKeys(json_encode(['inline_keyboard'=>[
        [['text'=>$buttonValues['declined'],'callback_data'=>"saosaoch"]]
        ]]));
...
if(preg_match('/^agencyApprove(\d+)_(\d+)/',$userInfo['step'],$match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    if(is_numeric($text)){
        editKeys(json_encode(['inline_keyboard'=>[
            [['text'=>$buttonValues['approved'],'callback_data'=>"saosaoch"]]
            ]]), $match[2]);
...
if(preg_match('/decline(\d+)_(\d+)/',$userInfo['step'],$match) && ($from_id == $admin || $userInfo['isAdmin'] == true) and $text != $buttonValues['cancel']){
    ...
    editKeys(
	    ...
	    [['text'=>"لغو شد ❌",'callback_data'=>"saosaoch"]]
	    ]]) ,$match[2]);
...
if($data == 'dayPlanSettings' and ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keyboard = [];
    $keyboard[] = [['text'=>"حذف",'callback_data'=>"saosaoch"],['text'=>"قیمت",'callback_data'=>"saosaoch"],['text'=>"تعداد روز",'callback_data'=>"saosaoch"]];
...
if($data == 'volumePlanSettings' and ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keyboard = [];
    $keyboard[] = [['text'=>"حذف",'callback_data'=>"saosaoch"],['text'=>"قیمت",'callback_data'=>"saosaoch"],['text'=>"مقدار حجم",'callback_data'=>"saosaoch"]];
...
if(preg_match('/^closeTicket_(\d+)/',$data,$match) and  $from_id != $admin){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"$from_id",'callback_data'=>"saosaoch"],
            ['text'=>"آیدی کاربر",'callback_data'=>'saosaoch']
        ],
        [
            ['text'=>$first_name??" ",'callback_data'=>"saosaoch"],
            ['text'=>"اسم کاربر",'callback_data'=>'saosaoch']
        ],
        [
            ['text'=>"$title",'callback_data'=>'saosaoch'],
            ['text'=>"عنوان",'callback_data'=>'saosaoch']
        ],
        [
            ['text'=>"$category",'callback_data'=>'saosaoch'],
            ['text'=>"دسته بندی",'callback_data'=>'saosaoch']
        ],
        ]]);
...
if(preg_match("/^rate_+([0-9])+_+([0-9])/",$data,$match)){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"رای تیکت",'callback_data'=>"saosaoch"]
            ],
        ]]);
...
if($data=='ticketsCategory' and ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $stmt->close();
    $keys = array();
    $keys[] = [['text'=>"حذف",'callback_data'=>"saosaoch"],['text'=>"دسته بندی",'callback_data'=>"saosaoch"]];
    
    if($ticketCategory->num_rows>0){
        while($row = $ticketCategory->fetch_assoc()){
            $rowId = $row['id'];
            $ticketName = $row['value'];
            $keys[] = [['text'=>"❌",'callback_data'=>"delTicketCat_$rowId"],['text'=>$ticketName,'callback_data'=>"saosaoch"]];
        }
    }else{
...
if(preg_match('/^closeTicket_(\d+)/',$data,$match) and  ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    editKeys(json_encode(['inline_keyboard'=>[
        [['text'=>"تیکت بسته شد",'callback_data'=>"saosaoch"]]
        ]]));
    ...
    while($row = $serversList->fetch_assoc()){
        ...
        if($response->success){
            ...
            if(strpos($list, $text)){
                ...
                $keys = json_encode(['inline_keyboard'=>[
                [
                    ['text'=>$state??" ",'callback_data'=>"saosaoch"],
                    ['text'=>"🔘 وضعیت اکانت 🔘",'callback_data'=>"saosaoch"],
                    ],
                [
					['text'=>$remark??" ",'callback_data'=>"saosaoch"],
                    ['text'=>"« نام اکانت »",'callback_data'=>"saosaoch"],
                    ],
                [
                    ['text'=>$upload?? " ",'callback_data'=>"saosaoch"],
                    ['text'=>"√ آپلود √",'callback_data'=>"saosaoch"],
                    ],
                [
                    ['text'=>$download??" ",'callback_data'=>"saosaoch"],
                    ['text'=>"√ دانلود √",'callback_data'=>"saosaoch"],
                    ],
                [
                    ['text'=>$total??" ",'callback_data'=>"saosaoch"],
                    ['text'=>"† حجم کلی †",'callback_data'=>"saosaoch"],
                    ],
                [
                    ['text'=>$leftMb??" ",'callback_data'=>"saosaoch"],
                    ['text'=>"~ حجم باقیمانده ~",'callback_data'=>"saosaoch"],
                    ],
                [
                    ['text'=>$expiryTime??" ",'callback_data'=>"saosaoch"],
                    ['text'=>"تاریخ اتمام",'callback_data'=>"saosaoch"],
                    ],
                [
                    ['text'=>$expiryDay??" ",'callback_data'=>"saosaoch"],
                    ['text'=>"تعداد روز باقیمانده",'callback_data'=>"saosaoch"],
                    ],
...
if($data == 'backplan' and ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keyboard[] = [['text'=>"➖➖➖",'callback_data'=>"saosaoch"]];
...
if(($data=="editCustomPlan" || preg_match('/^editCustom(gbPrice|dayPrice)/',$userInfo['step'],$match)) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ...
            ['text'=>"هزینه هر گیگ",'callback_data'=>"saosaoch"]
            ],
        [
            ['text'=>$dayPrice,'callback_data'=>"editCustomdayPrice"],
            ['text'=>"هزینه هر روز",'callback_data'=>"saosaoch"]
            ],
        [
            ['text'=>$buttonValues['back_button'],'callback_data'=>"backplan"]
            ]
...
if(preg_match('/^saosaoplanacclist(\d+)/',$data,$match) and ($from_id == $admin || $userInfo['isAdmin'] == true)){
...
if(preg_match('/^saosaoplandelete(\d+)/',$data,$match) and ($from_id == $admin || $userInfo['isAdmin'] == true)){
...
if(preg_match('/^saosaoplanname(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    setUser($data);
    delMessage();
    sendMessage("🔅 یه اسم برا پلن جدید انتخاب کن:",$cancelKey);exit;
}
if(preg_match('/^saosaoplanname(\d+)/',$userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
...
if(preg_match('/^saosaoplanslimit(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    setUser($data);
    delMessage();
    sendMessage("🔅 ظرفیت جدید برای پلن انتخاب کن:",$cancelKey);exit;
}
if(preg_match('/^saosaoplanslimit(\d+)/',$userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
...
if(preg_match('/^saosaoplansinobundid(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    setUser($data);
    delMessage();
    sendMessage("🔅 سطر جدید برای پلن انتخاب کن:",$cancelKey);exit;
}
if(preg_match('/^saosaoplansinobundid(\d+)/',$userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
...
if(preg_match('/^saosaoplaneditdes(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    setUser($data);
    delMessage();
    sendMessage("🎯 توضیحاتت رو برام وارد کن:",$cancelKey);exit;
}
if(preg_match('/^saosaoplaneditdes(\d+)/',$userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
...
if(preg_match('/^saosaoplanrial(\d+)/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    setUser($data);
    delMessage();
    sendMessage("🎯 شیطون قیمت و گرون کردی 😂 ، خب قیمت جدید و بزن ببینم :",$cancelKey);exit;
}
if(preg_match('/^saosaoplanrial(\d+)/',$userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true)&& $text != $buttonValues['cancel']){
    if(is_numeric($text)){
...
if(preg_match('/^saosaocategorydelete(\d+)_(\d+)/',$data, $match) and ($from_id == $admin || $userInfo['isAdmin'] == true)){
...
if(preg_match('/^saosaocategoryedit/',$data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    setUser($data);
    delMessage();
    sendMessage("〽️ یه اسم جدید برا دسته بندی انتخاب کن:",$cancelKey);exit;
}
if(preg_match('/saosaocategoryedit(\d+)_(\d+)/',$userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
...
if(preg_match('/^saosaodeleteserver(\d+)/',$data,$match) and ($from_id == $admin || $userInfo['isAdmin'] == true)){
