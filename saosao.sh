...
▒█▄▄▄█ ▒█░▒█ ▒█▄▄▄█ ▒█▄▄▄█ ▒█░▒█ ▒█▄▄▄█ 　 ▄▀▒▀▄ ░▀▄▄▀ ▄█▄
\033[0m"
echo -e "    \e[31mTelegram Channel: \e[34m@saosaoch\033[0m | \e[31mTelegram Group: \e[34m@saosaodev\033[0m\n"

#sleep
echo -e "\e[32mInstalling SaoSao script ... \033[0m\n"
sleep 5
...
echo -e "\n\033[33msaosao config and script have been installed successfully\033[0m"
...
echo "Folder created successfully!"
 ...
 cd /var/www/html/
 wget -O saosaopanel.zip https://github.com/hosseinabdinasab/saosaoxui/releases/download/1.1.0/saosaopanel.zip
...
if [ ! -d "/root/confsaosao" ]; then

    sudo mkdir /root/confsaosao
    
    sleep 1
    
    touch /root/confsaosao/dbrootsaosao.txt
    sudo chmod -R 777 /root/confsaosao/dbrootsaosao.txt
    sleep 1
    ...
    echo "${ASAS}user = 'root';" >> /root/confsaosao/dbrootsaosao.txt
    echo "${ASAS}pass = '${randomdbpasstxt}';" >> /root/confsaosao/dbrootsaosao.txt
    #echo "${ASAS}paths = '${RANDOM_CODE}';" >> /root/confsaosao/dbrootsaosao.txt
...
# update cron
PATHS=$(cat /root/confsaosao/dbrootsaosao.txt | grep '$path' | cut -d"'" -f2)
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/saosaoxui/settings/messagesaosao.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/saosaoxui/settings/rewardReport.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/saosaoxui/settings/warnusers.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/saosaoxui/settings/gift2all.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "*/3 * * * * curl https://${DOMAIN_NAME}/saosaoxui/settings/tronChecker.php >/dev/null 2>&1") | sort - | uniq - | crontab -
...
ROOT_PASSWORD=$(cat /root/confsaosao/dbrootsaosao.txt | grep '$pass' | cut -d"'" -f2)
ROOT_USER="root"
...
wait
    ...
    if [[ $(mysql -u root -p$ROOT_PASSWORD -e "SHOW DATABASES LIKE 'saosao'") ]]; then
        clear
        ...
        file_path="/var/www/html/saosaoxui/baseInfo.php"
        ...
        # print file
        echo -e "<?php" >> /var/www/html/saosaoxui/baseInfo.php
        echo -e "error_reporting(0);" >> /var/www/html/saosaoxui/baseInfo.php
        echo -e "${ASAS}botToken = '${YOUR_BOT_TOKEN}';" >> /var/www/html/saosaoxui/baseInfo.php
        echo -e "${ASAS}dbUserName = '${dbuser}';" >> /var/www/html/saosaoxui/baseInfo.php
        echo -e "${ASAS}dbPassword = '${dbpass}';" >> /var/www/html/saosaoxui/baseInfo.php
        echo -e "${ASAS}dbName = '${dbname}';" >> /var/www/html/saosaoxui/baseInfo.php
        echo -e "${ASAS}botUrl = 'https://${YOUR_DOMAIN}/saosaoxui/';" >> /var/www/html/saosaoxui/baseInfo.php
        echo -e "${ASAS}admin = ${YOUR_CHAT_ID};" >> /var/www/html/saosaoxui/baseInfo.php
        echo -e "?>" >> /var/www/html/saosaoxui/baseInfo.php

        sleep 1

        curl -F "url=https://${YOUR_DOMAIN}/saosaoxui/bot.php" "https://api.telegram.org/bot${YOUR_BOT_TOKEN}/setWebhook"
        MESSAGE="✅ The saosao bot has been successfully installed! @saosaoch"
        curl -s -X POST "https://api.telegram.org/bot${YOUR_BOT_TOKEN}/sendMessage" -d chat_id="${YOUR_CHAT_ID}" -d text="$MESSAGE"
        
        
        sleep 1
        
        url="https://${YOUR_DOMAIN}/saosaoxui/createDB.php"
        curl $url
        ...
        sudo rm /var/www/html/saosaoxui/createDB.php
